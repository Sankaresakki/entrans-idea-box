// Shared helpers for Vercel API routes (extracted from server.ts)
...
export function getAreaContext(area: string): string[] {
...
export function refineTitle(title: string, area: string): string {
  const cleaned = toTitleCase(title);
  const prefixes: Record<string, string> = {
    "Water Treatment": "AquaInnovate:", "Process Efficiency": "OptiFlow:",
    "Cost Reduction": "ValueEdge:", "Safety & Compliance": "SafeGuard:",
    "Digital & Automation": "DigiCore:", "Sustainability": "GreenLoop:",
    "Customer Experience": "CX360:", "General Innovation": "IonForward:",
  };
  const prefix = prefixes[area] || "IonForward:";
  if (cleaned.toLowerCase().startsWith(prefix.toLowerCase().replace(":", ""))) return cleaned;
  return `${prefix} ${cleaned}`;
}

export function refineProblem(problem: string, area: string): string {
...
export function refineSolution(solution: string, area: string): string {
  const keywords = getAreaContext(area);
  const k1 = keywords[0];
...
export function refineImpact(impact: string, area: string): string {
...
export function generateSuggestions(area: string): string[] {
...
export function generateEvalScores(
...
export function generateFeedback(
  scores: ReturnType<typeof generateEvalScores>,
  area: string,
  stage: string
): string {
  const avg = Math.round(Object.values(scores).reduce((a, b) => a + b, 0) / 6);
  const stageLabel = stage === "CIRC" ? "C-IRC" : "B-IRC";
  if (avg >= 82) return `This ${area} proposal demonstrates strong technical grounding and clear enterprise relevance. The ${stageLabel} panel finds the feasibility and scalability dimensions particularly compelling. Recommend fast-tracking to the next stage with a formal 60-day pilot plan and dedicated budget allocation.`;
  if (avg >= 72) return `The idea presents a viable ${area} intervention with good alignment to Ion Exchange's strategic priorities. The ${stageLabel} panel suggests strengthening the quantified impact projections and de-risking the implementation timeline before the next stage review. Overall outlook is positive.`;
